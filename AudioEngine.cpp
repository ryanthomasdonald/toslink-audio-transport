...
void initAudioSystem() {
  AudioMemory(48); 
  audioMixerL.gain(0, 1.0); audioMixerL.gain(1, 0.0);
  audioMixerR.gain(0, 1.0); audioMixerR.gain(1, 0.0);
}
...
void playFreshAlbumStart() {
  ...
  activeEngineIsA = true; nextTrackPreLaunched = false;
  ...
  if (activeEngineIsA) {
    ...
    if (!playWav1.isPlaying()) {
      if (nextTrackIndex < totalTracks) {
        AudioNoInterrupts();
        audioMixerL.gain(0, 0.0); audioMixerL.gain(1, 1.0); 
        audioMixerR.gain(0, 0.0); audioMixerR.gain(1, 1.0);
        AudioInterrupts();
