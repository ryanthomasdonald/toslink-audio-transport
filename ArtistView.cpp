...
void drawArtistView() {
  ...
  tft.setTextColor(ST7735_CYAN); tft.setTextSize(2); tft.setCursor(15, 13); tft.print("SELECT ARTIST");
  ...
  if (libraryArtistCount == 0) {
    ...
    for (int i = 0; i < 5; i++) {
      int itemIndex = menuScrollOffset + i; if (itemIndex >= libraryArtistCount) break; 
      ...
      tft.setTextColor(ST7735_WHITE); tft.setCursor(45, itemBoxY + 9); tft.print(library[itemIndex].name);
    }
  }
}
  ...
  if (currentTouch && !lastTouchState) {
    if (touchX >= 370 && touchX <= 465) {
      if (touchY >= 60 && touchY <= 125) { // BACK
        AudioNoInterrupts(); currentUIState = STATE_PLAYER; drawAudioDashboard();
        extern uint32_t lastUpdatedSecond; extern uint16_t lastProgressPixelWidth;
        lastUpdatedSecond = 999999; lastProgressPixelWidth = 0;
        updateTrackWindow(currentTrackIndex + 1, trackQueue[currentTrackIndex]);
        if (isMediaPlaying) updatePlayPauseButtonLabel("PAUSE", 0xD4A0); else updatePlayPauseButtonLabel("PLAY", 0x03E0);
        AudioInterrupts();
      }
      else if (touchY >= 140 && touchY <= 205) { if (menuScrollOffset >= 5) { menuScrollOffset -= 5; drawArtistView(); } }
      else if (touchY >= 220 && touchY <= 285) { if (menuScrollOffset + 5 < libraryArtistCount) { menuScrollOffset += 5; drawArtistView(); } }
    }
    for (int i = 0; i < 5; i++) {
      int itemIndex = menuScrollOffset + i; if (itemIndex >= libraryArtistCount) break;
      int itemBoxY = 60 + (i * 50);
      if (touchX >= 15 && touchX <= 355 && touchY >= (itemBoxY - 4) && touchY <= (itemBoxY + 38)) {
        selectedArtistIndex = itemIndex; menuScrollOffset = 0;
