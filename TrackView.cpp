...
void drawTrackView() {
  ...
  tft.setTextColor(ST7735_CYAN); tft.setTextSize(2); tft.setCursor(15, 13); tft.print(library[selectedArtistIndex].albums[selectedAlbumIndex].name);
  ...
  if (trackCount == 0) {
    ...
    for (int i = 0; i < 5; i++) {
      int itemIndex = menuScrollOffset + i; if (itemIndex >= trackCount) break; 
      ...
      tft.setTextColor(ST7735_WHITE); tft.setCursor(45, itemBoxY + 9);
...
void processTrackViewTouch() {
  ...
  if (currentTouch && !lastTouchState) {
    if (touchX >= 370 && touchX <= 465) {
      // 🚀 FIXED: Skip the redundant SD re-scan, shift state pointer in RAM and redraw silently
      if (touchY >= 60 && touchY <= 125) { 
        menuScrollOffset = 0; 
        currentMenuLevel = LEVEL_ALBUMS; 
        drawAlbumView(); 
      }
      else if (touchY >= 140 && touchY <= 205) { if (menuScrollOffset >= 5) { menuScrollOffset -= 5; drawTrackView(); } }
      else if (touchY >= 220 && touchY <= 285) { if (menuScrollOffset + 5 < trackCount) { menuScrollOffset += 5; drawTrackView(); } }
    }
    for (int i = 0; i < 5; i++) {
      int itemIndex = menuScrollOffset + i; if (itemIndex >= trackCount) break;
      int itemBoxY = 60 + (i * 50);
      if (touchX >= 15 && touchX <= 355 && touchY >= (itemBoxY - 4) && touchY <= (itemBoxY + 38)) {
        ...
        currentArtistFolder = persistArtistPath.c_str(); currentAlbumFolder = persistAlbumPath.c_str();
        ...
        } else { activeArtworkLoaded = false; }

        scanCurrentAlbumFolder(); currentTrackIndex = itemIndex; 
        
        extern UIState currentUIState; currentUIState = STATE_PLAYER;
        drawAudioDashboard(); playFreshAlbumStart(); break;
      }
    }
  }
